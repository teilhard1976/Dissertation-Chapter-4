...
```{r}
Plot1 <- ggplot(gapminder, aes(x = year, y = lifeExp, color = continent)) +
    geom_point()
Plot1 + geom_smooth(se=FALSE)
...
```{r}
Plot1 <- ggplot(gapminder, aes(year, lifeExp, color = continent)) +
    geom_point()
```
...
```{r}
PlotA <- ggplot(gapminder, aes(year, lifeExp)) +
    facet_wrap(~ continent) +
  geom_point() +
    geom_smooth(se = FALSE, span=0.99)
PlotA
```
...
```{r}
plotB <- ggplot(gapminder, aes(year, pop, group=country, colour=gdpPercap)) +
geom_line(alpha=0.2)
plotB
```
